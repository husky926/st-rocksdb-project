...
void ExtractStFileBoundsFromTableProperties(const TableProperties& tp,
                                            FileMetaData* meta) {
  if (meta == nullptr) {
    return;
  }
  ...
  if (it == tp.user_collected_properties.end()) {
    return;
  }
  Slice s(it->second);
  SpatioTemporalBlockMeta m{};
  Status st = DecodeSpatioTemporalIndexTail(&s, &m);
  if (!st.ok() || !s.empty()) {
    return;
  }
  meta->has_st_file_meta = true;
  meta->st_file_meta = m;
}
