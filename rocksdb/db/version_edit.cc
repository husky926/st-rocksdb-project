...
void VersionEdit::EncodeToNewFile4(const FileMetaData& f, int level,
  ...
  if (f.has_st_file_meta) {
    PutVarint32(dst, NewFileCustomTag::kExperimentalStFileBounds);
    std::string st_blob;
    AppendSpatioTemporalIndexTail(&st_blob, f.st_file_meta);
    PutLengthPrefixedSlice(dst, Slice(st_blob));
  }
    ...
    while (true) {
      ...
      switch (custom_tag) {
        ...
        case kMaxTimestamp:
          ...
          break;
        case kExperimentalStFileBounds: {
          Slice st_slice = field;
          Status stst = DecodeSpatioTemporalIndexTail(&st_slice, &f.st_file_meta);
          if (!stst.ok() || !st_slice.empty()) {
            return "invalid experimental st file bounds";
          }
          f.has_st_file_meta = true;
          break;
        }
        default:
