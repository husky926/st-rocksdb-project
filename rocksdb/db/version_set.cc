...
struct FileLevelEvalTimer {
  const ReadOptions::ExperimentalSpatioTemporalPruneScan& q;
  std::chrono::steady_clock::time_point t0;
  explicit FileLevelEvalTimer(
      const ReadOptions::ExperimentalSpatioTemporalPruneScan& in_q)
      : q(in_q), t0(std::chrono::steady_clock::now()) {}
  ~FileLevelEvalTimer() {
    if (q.file_level_eval_ns != nullptr) {
      const auto t1 = std::chrono::steady_clock::now();
      const auto delta_ns =
          static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                    t1 - t0)
                                    .count());
      *q.file_level_eval_ns += delta_ns;
    }
    if (q.file_level_eval_count != nullptr) {
      ++(*q.file_level_eval_count);
    }
  }
};

inline STPruneResult StFileOverlapWithExperimentalPruneScan(
    const FileMetaData* f, const ReadOptions& ro) {
  const auto& q = ro.experimental_st_prune_scan;
  if (!q.enable || !q.file_level_enable || f == nullptr || !f->has_st_file_meta) {
    return STPruneResult::kIntersect;
  }
  // Files with range deletions are never skipped by file-level pruning, and we
  // conservatively avoid "contains" short-circuiting for them as well.
  if (f->num_range_deletions > 0) {
    return STPruneResult::kIntersect;
  }
  const auto& m = f->st_file_meta;
  // 1) Disjoint
  if (m.t_max < q.t_min || m.t_min > q.t_max || m.x_max < q.x_min ||
      m.x_min > q.x_max || m.y_max < q.y_min || m.y_min > q.y_max) {
    return STPruneResult::kDisjoint;
  }
  // 2) Contains: query fully covers file bounds
  if (q.t_min <= m.t_min && q.t_max >= m.t_max && q.x_min <= m.x_min &&
      q.x_max >= m.x_max && q.y_min <= m.y_min && q.y_max >= m.y_max) {
    return STPruneResult::kContains;
  }
  // 3) Partial overlap
  return STPruneResult::kIntersect;
}

inline bool StFileDisjointFromExperimentalPruneScanWithDiag(
    const FileMetaData* f, const ReadOptions& ro) {
  const auto& q = ro.experimental_st_prune_scan;
  FileLevelEvalTimer eval_timer(q);
  if (q.file_level_files_considered != nullptr) {
    ++(*q.file_level_files_considered);
  }
  if (!q.enable || !q.file_level_enable || f == nullptr || !f->has_st_file_meta) {
    if (q.file_level_files_missing_meta != nullptr) {
      ++(*q.file_level_files_missing_meta);
    }
    return false;
  }
  if (f->num_range_deletions > 0) {
    if (q.file_level_files_range_del_blocked != nullptr) {
      ++(*q.file_level_files_range_del_blocked);
    }
    return false;
  }
  const auto& m = f->st_file_meta;
  ...
  if (time_disjoint && q.file_level_files_time_disjoint != nullptr) {
    ++(*q.file_level_files_time_disjoint);
  }
  if (space_disjoint && q.file_level_files_space_disjoint != nullptr) {
    ++(*q.file_level_files_space_disjoint);
  }
  return time_disjoint || space_disjoint;
}

inline bool StFileDisjointFromExperimentalPruneScan(const FileMetaData* f,
                                                    const ReadOptions& ro) {
  return StFileDisjointFromExperimentalPruneScanWithDiag(f, ro);
}
...
class LevelIterator final : public InternalIterator {
 ...
 private:
  ...
  bool IsStContainsAt(size_t file_index) const;
  void PrewarmForwardContainsBatch(size_t start_file_index);
  ...
  InternalIterator* NewFileIterator() {
    ...
    effective_read_options_holder_ = read_options_;
    ReadOptions& effective_read_options = effective_read_options_holder_;
    const auto& q = read_options_.experimental_st_prune_scan;
    if (q.enable && q.block_level_enable && file_meta.file_metadata != nullptr &&
        file_meta.file_metadata->has_st_file_meta &&
        file_meta.file_metadata->num_range_deletions == 0) {
      const STPruneResult overlap =
          StFileOverlapWithExperimentalPruneScan(file_meta.file_metadata,
                                                 effective_read_options);
      effective_read_options.experimental_st_prune_scan.file_level_overlap_result =
          overlap;
      if (overlap == STPruneResult::kContains) {
        // Query fully covers the file ST bounds: skip all local (block/key)
        // pruning work for this file iterator.
        effective_read_options.experimental_st_prune_scan.block_level_enable =
            false;
        effective_read_options.experimental_st_prune_scan.key_level_enable =
            false;
      }
    }
    if (q.enable && q.file_level_enable && q.block_level_enable &&
        (q.key_level_enable || q.block_level_adaptive_gate_enable) &&
        (q.key_level_adaptive_gate_enable || q.block_level_adaptive_gate_enable) &&
        file_meta.file_metadata != nullptr &&
        file_meta.file_metadata->has_st_file_meta) {
      const auto& m = file_meta.file_metadata->st_file_meta;
      const uint32_t t_lo = std::max(m.t_min, q.t_min);
      const uint32_t t_hi = std::min(m.t_max, q.t_max);
      const double t_overlap =
          (t_hi >= t_lo) ? static_cast<double>(t_hi - t_lo + 1) : 0.0;
      const double t_span =
          static_cast<double>(std::max<uint32_t>(1, m.t_max - m.t_min + 1));
      const double t_ratio = t_overlap / t_span;

      const float ox_min = std::max(m.x_min, q.x_min);
      const float ox_max = std::min(m.x_max, q.x_max);
      const float oy_min = std::max(m.y_min, q.y_min);
      const float oy_max = std::min(m.y_max, q.y_max);
      const double ox = std::max(0.0, static_cast<double>(ox_max - ox_min));
      const double oy = std::max(0.0, static_cast<double>(oy_max - oy_min));
      const double overlap_area = ox * oy;
      const double file_area =
          std::max(1e-9, static_cast<double>(m.x_max - m.x_min) *
                             static_cast<double>(m.y_max - m.y_min));
      const double xy_ratio = overlap_area / file_area;
      const double overlap_score = t_ratio * xy_ratio;
      if (q.block_level_adaptive_gate_enable &&
          overlap_score >= q.block_level_disable_overlap_threshold) {
        // High overlap: local pruning overhead likely exceeds benefit.
        effective_read_options.experimental_st_prune_scan.block_level_enable =
            false;
        effective_read_options.experimental_st_prune_scan.key_level_enable =
            false;
      } else if (q.key_level_adaptive_gate_enable &&
                 overlap_score >= q.key_level_disable_overlap_threshold) {
        effective_read_options.experimental_st_prune_scan.key_level_enable =
            false;
      }
    }
    ...
    return table_cache_->NewIterator(
        effective_read_options, file_options_, icomparator_,
  ...
  mutable std::vector<char> file_skip_mask_ = {};
  mutable bool file_skip_mask_ready_ = false;
  // EXPERIMENTAL (fork): cached candidate SST list for forward scans when
  ...
  ReadOptions effective_read_options_holder_;
  mutable bool compact_st_meta_ready_ = false;
  mutable std::vector<uint8_t> compact_has_st_meta_ = {};
  mutable std::vector<uint8_t> compact_has_range_del_ = {};
  ...
  bool StFileDisjointFromExperimentalPruneScanAt(size_t i) const;
  ...
  size_t NextForwardFileIndexSkippingStPrune(size_t start) const {
    size_t i = start;
    const auto& q = read_options_.experimental_st_prune_scan;
    ...
    while (i < flevel_->num_files) {
      size_t next_i = i;
      if (false && TrySkipEightFilesWithAvx2(i, &next_i)) {
        i = next_i;
        continue;
      }
      if (false && q.enable && q.file_level_enable && file_time_upper_skip_mask_ready_ &&
          file_time_upper_skip_mask_[i]) {
        if (q.file_level_files_considered != nullptr) {
          ++(*q.file_level_files_considered);
        }
        if (q.file_level_files_time_disjoint != nullptr) {
      ...
      if (!StFileDisjointFromExperimentalPruneScanAt(i)) {
        break;
      }
      if (q.file_level_files_skipped != nullptr) {
        ++(*q.file_level_files_skipped);
...
void LevelIterator::EnsureCompactStMeta() const {
  if (compact_st_meta_ready_) {
    return;
  }
  compact_st_meta_ready_ = true;
  compact_has_st_meta_.assign(flevel_->num_files, 0);
  compact_has_range_del_.assign(flevel_->num_files, 0);
  compact_st_meta_.assign(flevel_->num_files, SpatioTemporalBlockMeta{});
  for (size_t i = 0; i < flevel_->num_files; ++i) {
    const FileMetaData* f = flevel_->files[i].file_metadata;
    if (f == nullptr) {
      continue;
    }
    compact_has_range_del_[i] = static_cast<uint8_t>(f->num_range_deletions > 0);
    if (!f->has_st_file_meta) {
      continue;
    }
    compact_has_st_meta_[i] = 1;
    compact_st_meta_[i] = f->st_file_meta;
  }
}

void LevelIterator::EnsureStCandidateFilesFromSkipMask() const {
  if (st_candidate_files_ready_) {
    return;
  }
  EnsureTimeBucketRTreeSkipMask();
  st_candidate_files_ready_ = true;
  st_candidate_files_.clear();
  st_candidate_files_.reserve(flevel_->num_files);
  for (size_t i = 0; i < flevel_->num_files; ++i) {
...
bool LevelIterator::TrySkipEightFilesWithAvx2(size_t i, size_t* next_i) const {
  (void)i;
  (void)next_i;
  return false;
#ifndef __AVX2__
  ...
  for (size_t k = 0; k < 8; ++k) {
    ...
    if (!compact_has_st_meta_[idx] ||
        compact_has_range_del_[idx] ||
        (file_time_upper_skip_mask_ready_ && file_time_upper_skip_mask_[idx])) {
      return false;
    }
  }

  auto t0 = std::chrono::steady_clock::now();
  alignas(32) float x_min_arr[8], x_max_arr[8], y_min_arr[8], y_max_arr[8];
  alignas(32) uint32_t t_min_arr[8], t_max_arr[8];
  for (size_t k = 0; k < 8; ++k) {
    const auto& m = compact_st_meta_[i + k];
    t_min_arr[k] = m.t_min;
    t_max_arr[k] = m.t_max;
    x_min_arr[k] = m.x_min;
    x_max_arr[k] = m.x_max;
  ...
  const __m256 q_x_min = _mm256_set1_ps(q.x_min);
  const __m256 q_x_max = _mm256_set1_ps(q.x_max);
  const __m256 q_y_min = _mm256_set1_ps(q.y_min);
  const __m256 q_y_max = _mm256_set1_ps(q.y_max);
  const __m256i q_t_min = _mm256_set1_epi32(static_cast<int>(q.t_min));
  const __m256i q_t_max = _mm256_set1_epi32(static_cast<int>(q.t_max));

  const __m256 x_min_v = _mm256_load_ps(x_min_arr);
  const __m256 x_max_v = _mm256_load_ps(x_max_arr);
  const __m256 y_min_v = _mm256_load_ps(y_min_arr);
  const __m256 y_max_v = _mm256_load_ps(y_max_arr);
  const __m256i t_min_v =
      _mm256_load_si256(reinterpret_cast<const __m256i*>(t_min_arr));
  const __m256i t_max_v =
      _mm256_load_si256(reinterpret_cast<const __m256i*>(t_max_arr));

  const __m256i time_lt =
      _mm256_cmpgt_epi32(q_t_min, t_max_v);  // t_max < q_t_min
  const __m256i time_gt =
      _mm256_cmpgt_epi32(t_min_v, q_t_max);  // t_min > q_t_max
  const __m256i time_disjoint_i = _mm256_or_si256(time_lt, time_gt);
  const __m256 time_disjoint_f = _mm256_castsi256_ps(time_disjoint_i);

  const __m256 x_lt = _mm256_cmp_ps(x_max_v, q_x_min, _CMP_LT_OQ);
  const __m256 x_gt = _mm256_cmp_ps(x_min_v, q_x_max, _CMP_GT_OQ);
  const __m256 y_lt = _mm256_cmp_ps(y_max_v, q_y_min, _CMP_LT_OQ);
  const __m256 y_gt = _mm256_cmp_ps(y_min_v, q_y_max, _CMP_GT_OQ);
  const __m256 space_disjoint = _mm256_or_ps(_mm256_or_ps(x_lt, x_gt),
                                             _mm256_or_ps(y_lt, y_gt));
  const __m256 disjoint = _mm256_or_ps(time_disjoint_f, space_disjoint);

  const int disjoint_mask = _mm256_movemask_ps(disjoint);
  if (disjoint_mask != 0xFF) {
    if (q.file_level_eval_ns != nullptr) {
      const auto t1 = std::chrono::steady_clock::now();
      *q.file_level_eval_ns += static_cast<uint64_t>(
          std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count());
    }
    return false;
  }

  const int time_mask = _mm256_movemask_ps(time_disjoint_f);
  const int space_mask = _mm256_movemask_ps(space_disjoint);
  if (q.file_level_files_considered != nullptr) {
    *q.file_level_files_considered += 8;
  }
  if (q.file_level_files_time_disjoint != nullptr) {
    *q.file_level_files_time_disjoint +=
        static_cast<uint64_t>(__popcnt(static_cast<unsigned int>(time_mask)));
  }
  if (q.file_level_files_space_disjoint != nullptr) {
    *q.file_level_files_space_disjoint +=
        static_cast<uint64_t>(__popcnt(static_cast<unsigned int>(space_mask)));
  }
  if (q.file_level_files_skipped != nullptr) {
    *q.file_level_files_skipped += 8;
  }
  if (q.file_level_eval_count != nullptr) {
    *q.file_level_eval_count += 8;
  }
  if (q.file_level_eval_ns != nullptr) {
    const auto t1 = std::chrono::steady_clock::now();
    *q.file_level_eval_ns += static_cast<uint64_t>(
...
bool LevelIterator::StFileDisjointFromExperimentalPruneScanAt(size_t i) const {
  const auto& q = read_options_.experimental_st_prune_scan;
  FileLevelEvalTimer eval_timer(q);
  if (q.file_level_files_considered != nullptr) {
    ++(*q.file_level_files_considered);
  }
  if (!q.enable || !q.file_level_enable || i >= flevel_->num_files) {
    if (q.file_level_files_missing_meta != nullptr) {
      ++(*q.file_level_files_missing_meta);
    }
    return false;
  ...
  EnsureCompactStMeta();
  if (!compact_has_st_meta_[i]) {
    if (q.file_level_files_missing_meta != nullptr) {
      ++(*q.file_level_files_missing_meta);
    }
    return false;
  }
  if (compact_has_range_del_[i]) {
    if (q.file_level_files_range_del_blocked != nullptr) {
      ++(*q.file_level_files_range_del_blocked);
    }
    return false;
...
void LevelIterator::EnsureStOrderByTMin() const {
  if (st_order_by_tmin_ready_) {
    return;
  }
  st_order_by_tmin_ready_ = true;
  EnsureCompactStMeta();
  st_order_by_tmin_.clear();
  st_order_by_tmin_.reserve(flevel_->num_files);
  for (size_t i = 0; i < flevel_->num_files; ++i) {
    if (!compact_has_st_meta_[i] ||
        compact_has_range_del_[i]) {
      continue;
    }
    st_order_by_tmin_.push_back(i);
  }
  std::sort(st_order_by_tmin_.begin(), st_order_by_tmin_.end(),
            [&](size_t a, size_t b) {
              return compact_st_meta_[a].t_min <
                     compact_st_meta_[b].t_min;
            });
}

void LevelIterator::EnsureTimeUpperBoundSkipMask() const {
  if (file_time_upper_skip_mask_ready_) {
    return;
  }
  file_time_upper_skip_mask_ready_ = true;
  file_time_upper_skip_mask_.assign(flevel_->num_files, 0);
  const auto& q = read_options_.experimental_st_prune_scan;
  if (!q.enable || !q.file_level_enable || q.file_level_time_bucket_rtree_enable) {
    return;
  }
  ...
  auto it = std::upper_bound(
      st_order_by_tmin_.begin(), st_order_by_tmin_.end(), q.t_max,
      [&](uint32_t tmax, size_t idx) {
        return tmax < compact_st_meta_[idx].t_min;
      });
  for (; it != st_order_by_tmin_.end(); ++it) {
    file_time_upper_skip_mask_[*it] = 1;
  }
}

void LevelIterator::EnsureTimeBucketRTreeSkipMask() const {
  if (file_skip_mask_ready_) {
    return;
  }
  file_skip_mask_ready_ = true;
  file_skip_mask_.assign(flevel_->num_files, 0);
  ...
  const uint32_t bucket_count = std::max<uint32_t>(1, q.file_level_time_bucket_count);
  const uint32_t leaf_size = std::max<uint32_t>(2, q.file_level_rtree_leaf_size);
  EnsureCompactStMeta();

  // 1) Gather global min/max t over prunable files (conservative: do not index
  //    files that may hold range tombstones or miss st_file_meta).
  bool has_any = false;
  uint32_t global_min_t = 0;
  uint32_t global_max_t = 0;
  for (size_t i = 0; i < flevel_->num_files; ++i) {
    if (!compact_has_st_meta_[i] ||
        compact_has_range_del_[i]) {
      continue;
    ...
    if (!has_any) {
      has_any = true;
      global_min_t = m.t_min;
      global_max_t = m.t_max;
    } else {
      global_min_t = std::min(global_min_t, m.t_min);
      global_max_t = std::max(global_max_t, m.t_max);
    }
  }
  if (!has_any) {
    return;
  ...
  // Avoid div-by-zero; bucket_width >= 1.
  const uint64_t span = static_cast<uint64_t>(global_max_t) -
                         static_cast<uint64_t>(global_min_t);
  const uint32_t bucket_width = static_cast<uint32_t>(span / bucket_count + 1);

  // 2) Time bucket -> list of file indices that intersect that bucket.
  std::vector<std::vector<size_t>> bucket_files(bucket_count);
  for (size_t i = 0; i < flevel_->num_files; ++i) {
    if (!compact_has_st_meta_[i] ||
        compact_has_range_del_[i]) {
      continue;
    ...
    const auto& m = compact_st_meta_[i];
    const uint32_t b_start = std::min<uint32_t>(
        bucket_count - 1, (m.t_min - global_min_t) / bucket_width);
    const uint32_t b_end = std::min<uint32_t>(
        bucket_count - 1, (m.t_max - global_min_t) / bucket_width);
    for (uint32_t b = b_start; b <= b_end; ++b) {
      bucket_files[b].push_back(i);
    }
  }

  const float q_x_min = q.x_min;
  const float q_x_max = q.x_max;
  const float q_y_min = q.y_min;
  const float q_y_max = q.y_max;

  // 3) R-tree-like spatial BVH over x/y rectangles, built per bucket on demand.
  struct SpatialNode {
    float x_min;
    float x_max;
    float y_min;
    ...
    std::vector<size_t> ids;  // Leaf: file indices.
  };

  auto boxes_disjoint_2d = [&](float ax_min, float ax_max, float ay_min,
                               float ay_max) -> bool {
    return ax_max < q_x_min || ax_min > q_x_max || ay_max < q_y_min ||
           ay_min > q_y_max;
  };

  auto build_tree = [&](auto&& self, std::vector<size_t> ids) -> std::unique_ptr<SpatialNode> {
    if (ids.empty()) {
      return nullptr;
    }
    auto node = std::make_unique<SpatialNode>();
    const auto& first_m = compact_st_meta_[ids.front()];
    node->x_min = first_m.x_min;
    node->x_max = first_m.x_max;
    node->y_min = first_m.y_min;
    node->y_max = first_m.y_max;
    for (size_t idx : ids) {
      if (!compact_has_st_meta_[idx] ||
          compact_has_range_del_[idx]) {
        continue;  // Shouldn't happen (we only index prunable files), be safe.
      }
      const auto& m = compact_st_meta_[idx];
      node->x_min = std::min(node->x_min, m.x_min);
      node->x_max = std::max(node->x_max, m.x_max);
      node->y_min = std::min(node->y_min, m.y_min);
      node->y_max = std::max(node->y_max, m.y_max);
    }
    ...
    const float x_extent = node->x_max - node->x_min;
    const float y_extent = node->y_max - node->y_min;
    const bool split_by_x = x_extent >= y_extent;

    std::sort(ids.begin(), ids.end(), [&](size_t a, size_t b) {
      const auto& ma = compact_st_meta_[a];
      const auto& mb = compact_st_meta_[b];
      const float ca = split_by_x ? (ma.x_min + ma.x_max) * 0.5f
                                  : (ma.y_min + ma.y_max) * 0.5f;
      const float cb = split_by_x ? (mb.x_min + mb.x_max) * 0.5f
                                  : (mb.y_min + mb.y_max) * 0.5f;
      return ca < cb;
    });

    const size_t mid = ids.size() / 2;
    std::vector<size_t> left_ids(ids.begin(), ids.begin() + mid);
    std::vector<size_t> right_ids(ids.begin() + mid, ids.end());

    node->left = self(self, std::move(left_ids));
    node->right = self(self, std::move(right_ids));
    return node;
  };

  // Avoid repeating per-file disjoint checks when one file appears in
  // multiple overlapping time buckets.
  std::vector<uint8_t> file_checked(flevel_->num_files, 0);

  auto query_tree = [&](auto&& self, const SpatialNode* node) -> void {
    if (node == nullptr) {
      return;
    }
    if (boxes_disjoint_2d(node->x_min, node->x_max, node->y_min, node->y_max)) {
      return;
    }
    if (node->left == nullptr && node->right == nullptr) {
      // Leaf: check full 3D disjointness (time + x/y AABB).
      for (size_t idx : node->ids) {
        if (file_checked[idx]) {
          continue;
        }
        file_checked[idx] = 1;
        ...
        const auto& m = compact_st_meta_[idx];
        const bool disjoint = m.t_max < q.t_min || m.t_min > q.t_max ||
                               m.x_max < q_x_min || m.x_min > q_x_max ||
                               m.y_max < q_y_min || m.y_min > q_y_max;
        if (disjoint) {
          file_skip_mask_[idx] = 1;
        }
      }
      return;
    }
  ...
  // Query the index for buckets that overlap the query time range.
  const uint32_t qb_start =
      (q.t_min <= global_min_t)
          ? 0
          : std::min<uint32_t>(bucket_count - 1,
                                (q.t_min - global_min_t) / bucket_width);
  const uint32_t qb_end =
      (q.t_max <= global_min_t)
          ? 0
          : std::min<uint32_t>(bucket_count - 1,
                                (q.t_max - global_min_t) / bucket_width);

  for (uint32_t b = qb_start; b <= qb_end; ++b) {
    if (bucket_files[b].empty()) {
      continue;
    }
    auto ids = bucket_files[b];  // copy for sort/build (small vectors)
    auto root = build_tree(build_tree, std::move(ids));
    query_tree(query_tree, root.get());
  }
}
...
bool LevelIterator::SkipEmptyFileForward() {
  ...
  while (!to_return_sentinel_ &&
    ...
    // Move to next file
    const size_t next_file =
        NextForwardFileIndexSkippingStPrune(file_index_ + 1);
    if (next_file >= flevel_->num_files ||
        KeyReachedUpperBound(file_smallest_key(next_file)) ||
        prefix_exhausted_) {
      SetFileIterator(nullptr);
      ClearRangeTombstoneIter();
      break;
    ...
    InitFileIterator(next_file);
    if (file_iter_.iter() != nullptr) {
      // Batch-prewarm consecutive kContains files to reduce repeated
      // iterator/table-cache setup overhead in dense forward scans.
      PrewarmForwardContainsBatch(file_index_);
    }
...
bool LevelIterator::IsStContainsAt(size_t file_index) const {
  if (file_index >= flevel_->num_files) {
    return false;
  }
  const auto& q = read_options_.experimental_st_prune_scan;
  if (!q.enable || !q.file_level_enable) {
    return false;
  }
  const auto* f = flevel_->files[file_index].file_metadata;
  if (f == nullptr || !f->has_st_file_meta || f->num_range_deletions > 0) {
    return false;
  }
  return StFileOverlapWithExperimentalPruneScan(f, read_options_) ==
         STPruneResult::kContains;
}

void LevelIterator::PrewarmForwardContainsBatch(size_t start_file_index) {
  // Keep this conservative to cap CPU/memory overhead.
  constexpr size_t kMaxBatchPrewarm = 3;
  const auto& q = read_options_.experimental_st_prune_scan;
  if (!q.enable || !q.file_level_enable) {
    return;
  }
  if (!q.file_level_contains_batch_prewarm_enable) {
    return;
  }
  if (!IsStContainsAt(start_file_index)) {
    return;
  }

  const size_t before = file_index_;
  size_t prev = start_file_index;
  size_t cur = start_file_index;
  size_t warmed = 0;
  while (warmed < kMaxBatchPrewarm) {
    cur = NextForwardFileIndexSkippingStPrune(cur + 1);
    if (cur >= flevel_->num_files) {
      break;
    }
    if (cur != prev + 1) {
      // Only batch consecutive files to mimic a virtual contiguous run.
      break;
    }
    if (!IsStContainsAt(cur)) {
      break;
    }
    if (prepared_iters_.find(cur) != prepared_iters_.end()) {
      prev = cur;
      ++warmed;
      continue;
    }
...
void Version::AddIteratorsForLevel(const ReadOptions& read_options,
  ...
  if (level == 0) {
    ...
    for (size_t i = 0; i < storage_info_.LevelFilesBrief(0).num_files; i++) {
      ...
      if (StFileDisjointFromExperimentalPruneScanWithDiag(file.file_metadata,
                                                          read_options)) {
        if (read_options.experimental_st_prune_scan.file_level_files_skipped !=
            nullptr) {
          ++(*read_options.experimental_st_prune_scan.file_level_files_skipped);
        }
        continue;
      }
      ...
      for (int level = 0; level < cfd->NumberLevels(); level++) {
        ...
        for (const auto& f : level_files) {
          ...
          edit.AddFile(level, f->fd.GetNumber(), f->fd.GetPathId(),
                       ...
                       f->max_timestamp, f->has_st_file_meta, f->st_file_meta);
        }
      }
