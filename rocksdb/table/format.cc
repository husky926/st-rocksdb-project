...
void IndexValue::EncodeTo(std::string* dst, bool have_first_key,
                          ...
                          bool append_st_meta_tail,
                          const SpatioTemporalBlockMeta* st_meta_arg) const {
  if (previous_handle) {
  ...
  if (append_st_meta_tail) {
    assert(st_meta_arg != nullptr);
    AppendSpatioTemporalIndexTail(dst, *st_meta_arg);
  } else {
    (void)st_meta_arg;
  }
}
...
Status IndexValue::DecodeFrom(Slice* input, bool have_first_key,
  ...
  has_st_meta = false;
  st_meta = SpatioTemporalBlockMeta{};
  if (decode_st_meta_tail) {
    if (input->size() >= kSpatioTemporalIndexTailBytes()) {
      Status ts = DecodeSpatioTemporalIndexTail(input, &st_meta);
      if (!ts.ok()) {
        return ts;
      }
      has_st_meta = true;
