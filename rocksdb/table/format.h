...
struct IndexValue {
  ...
  // Set when index value ends with experimental spatio-temporal tail; see
  // BlockBasedTableOptions::experimental_spatio_temporal_meta_in_index_value.
  bool has_st_meta = false;
  ...
  // When append_st_meta_tail is true, st_meta must be non-null; appends a
  // fixed-size tail after the standard IndexValue encoding.
  void EncodeTo(std::string* dst, bool have_first_key,
                const BlockHandle* previous_handle,
                bool append_st_meta_tail = false,
                const SpatioTemporalBlockMeta* st_meta_arg = nullptr) const;
  Status DecodeFrom(Slice* input, bool have_first_key,
                    const BlockHandle* previous_handle,
                    bool decode_st_meta_tail = false);
