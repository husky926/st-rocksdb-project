  ...
  if (table_magic_number == kBlockBasedTableMagicNumber) {
    ...
    BlockBasedTableFactory* bbtf =
        static_cast_with_check<BlockBasedTableFactory>(
            options_.table_factory.get());
    BlockBasedTableOptions bbto = *(bbtf->GetOptions<BlockBasedTableOptions>());
    auto& props = table_properties_->user_collected_properties;
    // Index values may include ST tails (fork); default reader bbto has this
    // false, which breaks IndexBlockIter::NextEntryOffset (value_ shrinks but
    // physical entry still includes the tail).
    if (props.find(kExperimentalStFileBoundsPropertyName()) != props.end()) {
      bbto.experimental_spatio_temporal_meta_in_index_value = true;
    }
    options_.table_factory = std::make_shared<BlockBasedTableFactory>(bbto);
    bbtf =
        static_cast_with_check<BlockBasedTableFactory>(
            options_.table_factory.get());
...
BlockBasedTable* SstFileDumper::GetBlockBasedTableOrNull() const {
  if (!table_reader_ ||
      !options_.table_factory->IsInstanceOf(TableFactory::kBlockBasedTableName())) {
    return nullptr;
  }
  return static_cast_with_check<BlockBasedTable>(table_reader_.get());
}
