  ...
  switch (index_type) {
    case BlockBasedTableOptions::kBinarySearch: {
      result = new ShortenedIndexBuilder(
          ...
          table_opt.experimental_spatio_temporal_meta_in_index_value);
      break;
    }
...
void ShortenedIndexBuilder::MergeInternalKeyIntoBlockStMeta(
    const Slice& internal_key) {
  if (!attach_st_meta_in_index_value_) {
    return;
  }
  ParsedInternalKey parsed;
  if (!ParseInternalKey(internal_key, &parsed, /*log_err_key=*/false).ok()) {
    return;
  }
  MergeSpatioTemporalMetaFromUserKey(parsed.user_key, &block_st_meta_accumulator_,
                                    &block_st_meta_accumulator_valid_);
}

void ShortenedIndexBuilder::AccumulateFileSpatioTemporalMetaForManifest(
    SpatioTemporalBlockMeta* acc, bool* acc_valid) const {
  if (!attach_st_meta_in_index_value_ || acc == nullptr || acc_valid == nullptr) {
    return;
  }
  if (file_st_meta_accumulator_valid_) {
    UnionSpatioTemporalBlockMeta(acc, acc_valid, file_st_meta_accumulator_);
  }
}

SpatioTemporalBlockMeta ShortenedIndexBuilder::FinalizeBlockStMetaForDataBlock(
    const Slice& last_key_in_current_block,
    const Slice* first_key_in_next_block) {
  (void)last_key_in_current_block;
  (void)first_key_in_next_block;
  SpatioTemporalBlockMeta out{};
  if (block_st_meta_accumulator_valid_) {
    out = block_st_meta_accumulator_;
  } else {
    // No st-encoded keys in this block: use bounds that intersect every finite
    // query window so experimental prune scan never skips the block.
    out.t_min = 0;
    out.t_max = UINT32_MAX;
    out.x_min = -std::numeric_limits<float>::infinity();
    out.x_max = std::numeric_limits<float>::infinity();
    out.y_min = -std::numeric_limits<float>::infinity();
    out.y_max = std::numeric_limits<float>::infinity();
    out.bitmap = UINT64_MAX;
  }
  block_st_meta_accumulator_valid_ = false;
  block_st_meta_accumulator_ = SpatioTemporalBlockMeta{};
  return out;
}
...
void PartitionedIndexBuilder::MakeNewSubIndexBuilder() {
  auto new_builder = std::make_unique<ShortenedIndexBuilder>(
      ...
      table_opt_.experimental_spatio_temporal_meta_in_index_value);
...
Status PartitionedIndexBuilder::Finish(
  ...
  if (UNLIKELY(entries_.empty())) {
    ...
    entry.value->must_use_separator_with_seq_.StoreRelaxed(
        must_use_separator_with_seq_.LoadRelaxed());
    {
      SpatioTemporalBlockMeta chunk{};
      bool chunk_valid = false;
      entry.value->AccumulateFileSpatioTemporalMetaForManifest(&chunk,
                                                               &chunk_valid);
      if (chunk_valid) {
        UnionSpatioTemporalBlockMeta(&merged_partition_st_meta_,
                                     &merged_partition_st_meta_valid_, chunk);
      }
    }
...
void PartitionedIndexBuilder::AccumulateFileSpatioTemporalMetaForManifest(
    SpatioTemporalBlockMeta* acc, bool* acc_valid) const {
  if (!merged_partition_st_meta_valid_ || acc == nullptr ||
      acc_valid == nullptr) {
    return;
  }
  UnionSpatioTemporalBlockMeta(acc, acc_valid, merged_partition_st_meta_);
}
