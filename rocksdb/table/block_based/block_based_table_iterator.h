...
class BlockBasedTableIterator : public InternalIteratorBase<Slice> {
 ...
 private:
  ...
  struct BlockHandleInfo {
    ...
    // Captured from IndexValue when this handle was enqueued (readahead path);
    // used when index_iter_ may not match the loaded data block.
    bool index_st_meta_valid = false;
    SpatioTemporalBlockMeta index_st_meta{};
  };
  ...
  // After SeekToFirst (not Seek(key)), skip user keys whose decoded ST point is
  // disjoint from experimental_st_prune_scan inside data blocks.
  bool st_prune_key_level_forward_active_ = false;
  ...
  void FindKeyForward();
  // True when key_level_boundary_blocks_only is on and current block's index
  // ST bounds are fully inside the query window (safe to skip per-key ST checks).
  bool StKeyLevelSkipBecauseBlockFullyContained();
