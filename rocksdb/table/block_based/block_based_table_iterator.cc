...
inline bool IsStLocalPruneShortCircuited(const ReadOptions& ro) {
  const auto& q = ro.experimental_st_prune_scan;
  return q.enable &&
         q.file_level_overlap_result == STPruneResult::kContains;
}

void AdvanceIndexPastPrunedForward(
    const ReadOptions& ro, InternalIteratorBase<IndexValue>* index_iter) {
  while (index_iter->Valid()) {
    if (!ro.experimental_st_prune_scan.enable ||
        !ro.experimental_st_prune_scan.block_level_enable ||
...
void BlockBasedTableIterator::SeekSecondPass(const Slice* target) {
  ...
  if (target == nullptr && read_options_.experimental_st_prune_scan.enable &&
      read_options_.experimental_st_prune_scan.key_level_enable &&
      !IsStLocalPruneShortCircuited(read_options_) && Valid()) {
    st_prune_key_level_forward_active_ = true;
  }
}
...
void BlockBasedTableIterator::SeekImpl(const Slice* target,
  ...
  st_prune_key_level_forward_active_ = false;
  ...
  if (need_seek_index) {
    ...
    if (target == nullptr &&
        !IsStLocalPruneShortCircuited(read_options_)) {
      AdvanceIndexPastPrunedForward(read_options_, index_iter_.get());
      if (!index_iter_->Valid()) {
        ResetDataIter();
        return;
      }
  ...
  if (!v.first_internal_key.empty() && !same_block &&
      ...
      (!read_options_.experimental_st_prune_scan.enable ||
       !read_options_.experimental_st_prune_scan.key_level_enable)) {
...
void BlockBasedTableIterator::FindKeyForward() {
  ...
  for (;;) {
    if (!block_iter_.Valid()) {
      // This is the only call site of FindBlockForward(), but it's extracted
      // into a separate method to keep FindKeyForward() short and likely to be
    ...
    if (!read_options_.experimental_st_prune_scan.enable ||
        !read_options_.experimental_st_prune_scan.key_level_enable ||
        IsStLocalPruneShortCircuited(read_options_) ||
        !st_prune_key_level_forward_active_) {
      return;
    }
    if (StKeyLevelSkipBecauseBlockFullyContained()) {
      return;
    }
    const auto& q = read_options_.experimental_st_prune_scan;
    if (q.key_level_keys_examined != nullptr) {
      ++(*q.key_level_keys_examined);
    }
    const Slice ukey = ExtractUserKey(block_iter_.key());
    if (!UserKeySpatioTemporalDisjointFromPruneScan(ukey, q)) {
      return;
    }
    if (q.key_level_keys_skipped_disjoint != nullptr) {
      ++(*q.key_level_keys_skipped_disjoint);
    }
    block_iter_.Next();
  }
}

bool BlockBasedTableIterator::StKeyLevelSkipBecauseBlockFullyContained() {
  const auto& q = read_options_.experimental_st_prune_scan;
  if (!q.enable || !q.key_level_enable ||
      !q.key_level_boundary_blocks_only_enable) {
    return false;
  }
  SpatioTemporalBlockMeta m{};
  bool has = false;
  if (DoesContainBlockHandles()) {
    if (block_handles_ == nullptr || block_handles_->empty()) {
      return false;
    }
    const auto& bh = block_handles_->front();
    has = bh.index_st_meta_valid;
    m = bh.index_st_meta;
  } else {
    if (!index_iter_->Valid()) {
      return false;
  ...
  return q.x_min <= m.x_min && q.x_max >= m.x_max &&
         q.y_min <= m.y_min && q.y_max >= m.y_max && q.t_min <= m.t_min &&
         q.t_max >= m.t_max;
}
...
void BlockBasedTableIterator::FindBlockForward() {
  ...
  do {
    ...
    if (!DoesContainBlockHandles()) {
      ...
      if (!IsStLocalPruneShortCircuited(read_options_)) {
        AdvanceIndexPastPrunedForward(read_options_, index_iter_.get());
      }
      if (!index_iter_->Valid()) {
        return;
...
void BlockBasedTableIterator::InitializeStartAndEndOffsets(
  ...
  if (read_curr_block) {
    if (!DoesContainBlockHandles()) {
      ...
      const IndexValue& iv_enqueue = index_iter_->value();
      block_handle_info.handle_ = iv_enqueue.handle;
      block_handle_info.SetFirstInternalKey(iv_enqueue.first_internal_key);
      block_handle_info.index_st_meta_valid = iv_enqueue.has_st_meta;
      if (iv_enqueue.has_st_meta) {
        block_handle_info.index_st_meta = iv_enqueue.st_meta;
      }
...
void BlockBasedTableIterator::BlockCacheLookupForReadAheadSize(
  ...
  while (index_iter_->Valid() && !is_index_out_of_bound_) {
    ...
    // data block in cache.
    BlockHandleInfo block_handle_info;
    const IndexValue& iv_ra = index_iter_->value();
    block_handle_info.handle_ = iv_ra.handle;
    block_handle_info.SetFirstInternalKey(iv_ra.first_internal_key);
    block_handle_info.index_st_meta_valid = iv_ra.has_st_meta;
    if (iv_ra.has_st_meta) {
      block_handle_info.index_st_meta = iv_ra.st_meta;
    }
