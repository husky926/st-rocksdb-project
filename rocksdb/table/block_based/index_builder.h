...
class IndexBuilder {
 public:
  ...
  virtual Slice AddIndexEntry(const Slice& last_key_in_current_block,
                              ...
                              const SpatioTemporalBlockMeta& st_meta = {}) = 0;
  ...
  // When experimental index ST tails are enabled, merges per-partition /
  // whole-index file-level bounds for `rocksdb.experimental.st_file_bounds`.
  virtual void AccumulateFileSpatioTemporalMetaForManifest(
      SpatioTemporalBlockMeta* /*acc*/, bool* /*acc_valid*/) const {}
...
class ShortenedIndexBuilder : public IndexBuilder {
 public:
  ShortenedIndexBuilder(
      ...
      double uniform_cv_threshold, bool attach_st_meta_in_index_value)
      : IndexBuilder(comparator, ts_sz, persist_user_defined_timestamps),
        ...
        attach_st_meta_in_index_value_(attach_st_meta_in_index_value) {
  ...
  void OnKeyAdded(const Slice& key,
    ...
    MergeInternalKeyIntoBlockStMeta(key);
  }
  ...
  void AddIndexEntryImpl(const Slice& separator_with_seq,
                         ...
                         bool skip_delta_encoding,
                         const SpatioTemporalBlockMeta& st_meta) {
    IndexValue entry(block_handle, first_internal_key);
    std::string encoded_entry;
    std::string delta_encoded_entry;
    const bool append_st = attach_st_meta_in_index_value_;
    const SpatioTemporalBlockMeta* st_ptr = append_st ? &st_meta : nullptr;
    entry.EncodeTo(&encoded_entry, include_first_key_, nullptr, append_st,
                   st_ptr);
    if (use_value_delta_encoding_ && !last_encoded_handle_.IsNull() &&
        !skip_delta_encoding) {
      entry.EncodeTo(&delta_encoded_entry, include_first_key_,
                     &last_encoded_handle_, append_st, st_ptr);
    } else {
  ...
  Slice AddIndexEntry(const Slice& last_key_in_current_block,
                      ...
                      std::string* separator_scratch,
                      bool skip_delta_encoding,
                      const SpatioTemporalBlockMeta& /*st_meta_from_caller*/)
      override {
  ...
  struct ShortenedPreparedIndexEntry : public PreparedIndexEntry {
    ...
    SpatioTemporalBlockMeta st_meta{};
  ...
  void PrepareIndexEntry(const Slice& last_key_in_current_block,
    ...
    entry->SaveFrom(separator, first_internal_key,
                    must_use_separator_with_seq_.LoadRelaxed());
    entry->st_meta = FinalizeBlockStMetaForDataBlock(last_key_in_current_block,
                                                     first_key_in_next_block);
    current_block_first_internal_key_.clear();
  }
  ...
  void AccumulateFileSpatioTemporalMetaForManifest(
      SpatioTemporalBlockMeta* acc, bool* acc_valid) const override;
  ...
  // Per-data-block ST summary from keys only. MergeInternalKeyIntoBlockStMeta is
  // called from OnKeyAdded; FinalizeBlockStMetaForDataBlock runs at each index
  // entry (AddIndexEntry / PrepareIndexEntry) and resets the accumulator.
  void MergeInternalKeyIntoBlockStMeta(const Slice& internal_key);
  SpatioTemporalBlockMeta FinalizeBlockStMetaForDataBlock(
      const Slice& last_key_in_current_block,
      const Slice* first_key_in_next_block);
  inline void MergeCompletedBlockIntoFileMeta(
      const SpatioTemporalBlockMeta& block) {
    if (!attach_st_meta_in_index_value_) {
      return;
    }
    UnionSpatioTemporalBlockMeta(&file_st_meta_accumulator_,
                                 &file_st_meta_accumulator_valid_, block);
  }
 ...
 private:
  ...
  const bool attach_st_meta_in_index_value_;
  ...
  bool block_st_meta_accumulator_valid_ = false;
  SpatioTemporalBlockMeta block_st_meta_accumulator_{};
  bool file_st_meta_accumulator_valid_ = false;
  SpatioTemporalBlockMeta file_st_meta_accumulator_{};
};
...
class HashIndexBuilder : public IndexBuilder {
 public:
  HashIndexBuilder(const InternalKeyComparator* comparator,
                   ...
                   bool attach_st_meta_in_index_value)
      : IndexBuilder(comparator, ts_sz, persist_user_defined_timestamps),
  ...
  Slice AddIndexEntry(const Slice& last_key_in_current_block,
    ...
    return primary_index_builder_.AddIndexEntry(
        ...
        separator_scratch, skip_delta_encoding, st_meta);
  }
  ...
  void AccumulateFileSpatioTemporalMetaForManifest(
      SpatioTemporalBlockMeta* acc, bool* acc_valid) const override {
    primary_index_builder_.AccumulateFileSpatioTemporalMetaForManifest(
        acc, acc_valid);
  }
...
class PartitionedIndexBuilder : public IndexBuilder {
 ...
 private:
  ...
  // Union of per-partition file-level ST bounds (for Manifest user property).
  bool merged_partition_st_meta_valid_ = false;
  SpatioTemporalBlockMeta merged_partition_st_meta_{};
};
}  // namespace ROCKSDB_NAMESPACE
