...
void BlockBasedTableBuilder::WritePropertiesBlock(
  ...
  if (LIKELY(ok())) {
    ...
    if (rep_->table_options.experimental_spatio_temporal_meta_in_index_value) {
      SpatioTemporalBlockMeta file_meta{};
      bool file_valid = false;
      rep_->index_builder->AccumulateFileSpatioTemporalMetaForManifest(
          &file_meta, &file_valid);
      if (file_valid) {
        std::string st_blob;
        AppendSpatioTemporalIndexTail(&st_blob, file_meta);
        rep_->props.user_collected_properties
            [kExperimentalStFileBoundsPropertyName()] = std::move(st_blob);
      }
    }
