...
// EXPERIMENTAL (fork): Tri-state overlap between a file/block ST bounds and
// a prune window.
enum class STPruneResult : uint8_t {
  kDisjoint = 0,   // No overlap (file/block can be skipped).
  kIntersect = 1,  // Partial overlap (needs finer-grained pruning).
  kContains = 2    // Query fully contains bounds (short-circuit local pruning).
};
class IODispatcher;
...
struct ReadOptions {
  ...
  // EXPERIMENTAL (fork): When iterating forward, skip loading data blocks whose
  // spatio-temporal index tail (see BlockBasedTableOptions::
  // experimental_spatio_temporal_meta_in_index_value) does not intersect this
  // axis-aligned window. Also skips opening SST files (LevelIterator) when
  // FileMetaData::has_st_file_meta and file-level bounds are disjoint from this
  // window (same axis test); files with range deletions are never skipped.
  // After SeekToFirst() and on Next(), also skips individual user keys whose
  // decoded ST point (same magic layout) is disjoint from the window, inside
  // blocks whose MBR still intersects the window.
  // Only applied after SeekToFirst() and on Next(); not applied to
  // Seek(key!=nullptr). User keys must use the st-meta prefix
  // (magic 0xE5 + t + lon + lat) so block-level meta is populated.
  struct ExperimentalSpatioTemporalPruneScan {
    bool enable = false;
    // Sub-features for ablation. When using `enable=true`, default behavior
    // preserves the original implementation by turning all sub-features on.
    // For strict experiments, set each sub-feature explicitly.
    bool file_level_enable = true;   // Skip opening SST files via Manifest/Version.
    bool block_level_enable = true;  // Skip data blocks via SST index tail.
    bool key_level_enable = true;    // Skip individual (t,x,y) keys inside blocks.
    // When true with key_level_enable, only run key-level pruning on blocks whose
    // index ST MBR is not fully contained by the query window (interior blocks
    // skip per-key checks). No effect when key_level_enable is false.
    bool key_level_boundary_blocks_only_enable = false;
    // EXPERIMENTAL (fork): Build a per-query in-memory index on SST file
    // spatio-temporal bounds:
    //   1) Partition files by coarse time buckets (global across the level).
    //   2) Inside each bucket, build an R-tree-like bounding-volume tree
    //      over (x_min,x_max,y_min,y_max) rectangles.
    // This affects only file-level skipping in forward iteration.
    bool file_level_time_bucket_rtree_enable = false;
    // Experimental: when true, prewarm a tiny consecutive kContains batch
    // in LevelIterator forward scans to reduce iterator setup overhead.
    ...
    bool block_level_adaptive_gate_enable = false;
    // Threshold on overlap score in [0,1]. If score >= threshold, disable
    // block-level and key-level pruning for that file iterator.
    float block_level_disable_overlap_threshold = 0.85f;
    // Experimental: adaptive gating for key-level pruning in Local+Global mode.
    // If enabled, implementation may disable key-level pruning for files whose
    // file-level overlap with query window is high (block-level still on).
    bool key_level_adaptive_gate_enable = false;
    // Threshold on overlap score in [0,1]. Higher score means query covers a
    // larger portion of file-level ST bounds. If score >= threshold, key-level
    // pruning is disabled for that file iterator.
    float key_level_disable_overlap_threshold = 0.6f;
    // Internal: optional counter for "actual skipped SST files" during forward
    // iteration (LevelIterator). When non-null, implementation increments it
    // each time a file is skipped due to file-level ST disjointness.
    uint64_t* file_level_files_skipped = nullptr;
    // Internal diagnostic counters for file-level prune decisions.
    uint64_t* file_level_files_considered = nullptr;
    uint64_t* file_level_files_missing_meta = nullptr;
    uint64_t* file_level_files_range_del_blocked = nullptr;
    uint64_t* file_level_files_time_disjoint = nullptr;
    uint64_t* file_level_files_space_disjoint = nullptr;
    // Internal diagnostic: number of file-level disjointness evaluations.
    uint64_t* file_level_eval_count = nullptr;
    // Internal diagnostic: total time spent in file-level evaluations (ns).
    uint64_t* file_level_eval_ns = nullptr;
    // Internal diagnostics: SST index / data-block pruning (block-level ST meta).
    // Incremented in BlockBasedTableIterator forward scans when block_level is on.
    ...
    // Internal: per-file overlap result computed at file-open time
    // (LevelIterator/TableCache). Default preserves legacy behavior.
    STPruneResult file_level_overlap_result = STPruneResult::kIntersect;
    uint32_t t_min = 0;
    uint32_t t_max = 0;
    float x_min = 0;
    float y_min = 0;
