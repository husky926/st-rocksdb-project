...
struct AdvancedColumnFamilyOptions {
  ...
  // [experimental]
  // If true, compaction may finish the current output SST before the next key
  // when Event Time (ST user key `t_sec`, see `st_meta_user_key.h`) crosses
  // configured thresholds. Intended for trajectory workloads: align physical
  // SST boundaries with business time buckets / max span. Non-ST keys do not
  // trigger this path. Default: false.
  bool experimental_compaction_event_time_split = false;
  // Max inclusive time span (t_max - t_min) among ST keys already in the
  // current output file, after including the next key; if exceeded, cut before
  // that key. 0 = disable this rule.
  uint32_t experimental_compaction_event_time_max_span = 0;
  // If non-zero, cut before a key whose floor(t / width) differs from the
  // last written ST key's bucket in the current output file.
  uint32_t experimental_compaction_event_time_bucket_width = 0;
  // If non-zero, require at least this many ST keys already written to the
  // current output file before allowing a split due to time bucket change.
  // This prevents tiny files when input oscillates around bucket boundaries.
  uint32_t experimental_compaction_event_time_min_keys_per_file = 0;
  // If non-zero, cut before adding another ST key when the file already holds
  // this many ST point keys.
  uint32_t experimental_compaction_event_time_max_point_keys = 0;
